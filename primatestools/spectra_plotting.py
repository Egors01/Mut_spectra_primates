...
class SpectraPlotTools():
    def __init__(self):
        self.rations_df = pd.DataFrame()
        pass
    def plot_heatmaps_short(self,m96,sp_name_to_col_name,pictures_sets_list,title='insert title'):
        ratios_df_single = self.create_ratio_table(m96=m96,
                                                   sp_name_to_col_name=sp_name_to_col_name)

        for i, pic in enumerate(pictures_sets_list):
            print(i)
            self.plot_heatmaps_from_ratios_list(ratios_table=ratios_df_single,
                                           pairs_on_image_list=pictures_sets_list[i],
                                           kelley_notation=True,
                                           title=title)
        return 0

    def create_ratio_table(self, m96, sp_name_to_col_name):
        ...
        for i, pair in enumerate(pairs_to_compare):
            ...
            relations_df[first + ' vs ' + second] = m96[colname_first] / m96[
                colname_second]
            relations_df[second + ' vs ' + first] = m96[colname_second] / m96[
                colname_first]
        return relations_df


    def create_heatmap_df_for_species(self,relations_df, species_column_name,
                                      kelley_notation=True):
    ...
    def plot_heatmaps_from_ratios_list(self,ratios_table, pairs_on_image_list, title='',
                                       kelley_notation=True, if_save=True,
                                       filename=''):
        colnames_to_plot = [pair[0] + ' vs ' + pair[1] for pair in
                            pairs_on_image_list]
        heatmaps_to_plot = [
            self.create_heatmap_df_for_species(relations_df=ratios_table,
    ...
    def plot_mutational_spectra(self, matrix, species_colnames, title='',
